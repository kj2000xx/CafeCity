...
public class StarRateView: UIView {
    ...
    // 星星顏色
    lazy var starColor = UIColor.yellow
    ...
    public init(frame:CGRect, totalStarCount:CGFloat, currentStarCount:CGFloat, starSpace:CGFloat) {
        ...
        let longTap = UILongPressGestureRecognizer(target: self, action: #selector(startTap))
        
        addGestureRecognizer(tap)
        addGestureRecognizer(longTap)
        
        unStarView = setupStarView("unStar")
        
        addSubview(unStarView)
        
        starView = setupStarView("star")
...
extension StarRateView{
    ...
    fileprivate func setupStarView(_ imageName:String) -> UIView {
        ...
        for _ in 0..<Int(totalStarCount) {
            ...
            if imageName == "star" {
                ...
                imageView.tintColor = self.starColor
    ...
    fileprivate func showStarRate(){
        ...
        if self.currentStarCount.isEqual(to: 0) {
        self.starView.frame = CGRect(x: 0, y: 0, width: 0, height: self.bounds.height)
        }
...
extension StarRateView{
    ...
    /// 点击评分
    @objc func startTap(_ tap:UIGestureRecognizer) {
